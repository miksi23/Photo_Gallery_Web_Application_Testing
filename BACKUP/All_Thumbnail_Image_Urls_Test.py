...
def scroll_and_count_all_images(browser, scroll_delay, max_scrolls):
    scroll_element = browser.find_element(By.TAG_NAME, 'body')
    ...
    while scrolls_done < max_scrolls:
        ...
        if scrolls_done == max_scrolls:
            ...
            print("Total images found", current_images_count)
            break

def test_thumbnail_image_urls():
    ...
    for index, image in enumerate(image_elements):
        try:
            ...
            if "background-image" in image_style:
                ...
                print(f"Image {index+1} - No image URL found")
        except NoSuchElementException:
            print(f"Image {index+1} - Element not found")

# simulate scroll down
scroll_and_count_all_images(browser, 0.5, 5)

test_thumbnail_image_urls()
